...
tcIdInfo ignore_prags name ty info 
  ...
  where
    ...
    tcPrag info (ND_HsStrictness str) = return (info `nd_setStrictnessInfo` str)
...
tcUnfolding name _ info (IfCoreUnfold stable if_expr)
  ...
  where
    ...
    is_bottoming = ND.isBottomingSig $ nd_strictnessInfo info              
...
tcIfaceWrapper name ty info arity get_worker
  = do  { mb_wkr_id <- forkM_maybe doc get_worker
        ; us <- newUniqueSupply
        ; dflags <- getDynFlags
        ; let mk_wrapper = \dfs t -> NWW.mkWrapper dfs t nd_strict_sig
