...
module NewWwLib ( mkWwBodies, mkWWstr, mkWorkerArgs ) where

#include "HsVersions.h"

import CoreSyn
import CoreUtils	( exprType )
import Id		( Id, idType, mkSysLocal, nd_idDemandInfo, nd_setIdDemandInfo,
...
mkWwBodies dflags fun_ty demands res_info one_shots
  = do	{ let arg_info = demands `zip` (one_shots ++ repeat False)
	...
	; return ([nd_idDemandInfo v | v <- work_call_args, isId v],
...
mk_wrap_arg uniq ty dmd one_shot 
  = set_one_shot one_shot (nd_setIdDemandInfo (mkSysLocal (fsLit "w") uniq ty) dmd)
  where
...
mkWWstr_one dflags arg
  ...
  where
    ...
    set_worker_arg_info worker_arg demand = set_one_shot (nd_setIdDemandInfo worker_arg demand)
